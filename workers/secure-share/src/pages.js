 ...
 * (CREATE_JS, REVEAL_JS). The server never sees plaintext or the key.
 */
...
export function renderCreatePage() {
  const body =
    ...
    '<p class="lede">Share this link with the recipient over your normal channel. Anyone with ' +
    "the full link can view the secret one time.</p>" +
    '<div class="outbox">' +
    ...
    "</div>" +
    "</div>";
  return shell("Forge RPA Secure Share", body, "/create.js");
}
...
export function renderViewPage(env, mode) {
  if (mode === "notfound") {
    const nf =
      ...
      '<div class="btn-row">' +
      '<a href="/" class="btn secondary">Create a Secure Link</a>' +
      "</div>" +
      "</div>";
  ...
  const body =
    ...
    "once. The moment you reveal it, it is permanently destroyed on the server. Have somewhere " +
...
// this is where AES-256-GCM encryption and decryption actually happen.
// ---------------------------------------------------------------------------
...
export const CREATE_JS = `(function(){
  ...
  function showResult(link,ttlSec){
  ...
  function onCreate(){
    ...
    (async function(){
      try{
        ...
        var ctBuf=await crypto.subtle.encrypt({name:"AES-GCM",iv:iv},key,enc.encode(secret));
        ...
        var link=location.origin+"/s#"+data.id+"."+b64url(rawKey);
        $("secret").value="";
        showResult(link,data.ttl||ttl);
      }catch(e){
...
export const REVEAL_JS = `(function(){
  ...
  function onReveal(){
    ...
    if(!hash||dot<1){setStatus("This link is missing its decryption key. Ask the sender to resend the full link.","err");return;}
    var id=hash.slice(0,dot);
    var keyB64=hash.slice(dot+1);
    if(!id||!keyB64){setStatus("This link is incomplete. Ask the sender to resend it.","err");return;}
    ...
    (async function(){
      try{
        ...
        if(res.status===429){btn.disabled=false;btn.textContent="Reveal Secret";setStatus("Too many attempts. Wait a moment and try again.","err");return;}
        if(!res.ok){btn.disabled=false;btn.textContent="Reveal Secret";setStatus("Something went wrong. Please try again.","err");return;}
        var data=await res.json();
        var key=await crypto.subtle.importKey("raw",fromB64url(keyB64),{name:"AES-GCM"},false,["decrypt"]);
        var ptBuf;
        try{
          ptBuf=await crypto.subtle.decrypt({name:"AES-GCM",iv:fromB64(data.iv)},key,fromB64(data.ct));
        }catch(e){
          hideReveal();
        ...
        // Strip the key from the address bar so it is not left on screen.
