...
metric: "40+ accounts, a full day per cycle returned"
shortDescription: "Bank reconciliation automation that works BlackLine the way the accountant did, without replacing the tool or changing approvals"
challenge: "Every cycle, one accountant spent a full day on more than 40 bank accounts: open each bank statement PDF, read the ending balance, key it into the BlackLine reconciliation, compare it to the book balance, and close the item. The work was pure relay, the same number read in one place and typed into another, forty times over, with the close waiting on it."
approach: "We automated BlackLine itself rather than working around it. The automation reads the ending balance off each statement PDF, enters it into the matching BlackLine reconciliation, compares, and closes the item when it ties. Anything that does not tie stays with a person. Nothing about the control changed: the same approvals, the same reviewers, the same audit trail, in the reconciliation tool the team already owned."
outcome:
